...
def check_clf(clf, signature, v=0):
    ...
    if v>=2: wrap_print("subordinate relation: {}".format(pr_2rel(sub_rel)))
...
def pr_box(box, indent=0):
    ...
    #discourse referents
    wrap_print("{} {} {}".format(ind, '| ', ' '.join(box.refs)))
    ...
    #    for r in box.rels:
    #        print ind, '[ ', ' '.join(r), ' ]'
    if box.cond_refs: wrap_print("{} {} {} {}".format(ind, '{ ', ' '.join(box.cond_refs), ' }'))
    #if box.rel_boxes: print ind, '[{ ', ' '.join(box.rel_boxes), ' ]]'
    if box.subs: wrap_print("{} {} {} {}".format(ind, '< ', ' '.join(box.subs), ' >'))
...
def connected_boxes(elements, rels, v=0):
    ...
    if diff:
        ...
        if v >= 2: wrap_print("Boxes {} are connected".format(pr_set(elements)))
