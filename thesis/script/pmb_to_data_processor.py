...
from data_transformer import parse_research_data, transform_data_to_input_variables, flush_clean_drs_data
...
def file_to_clfs(filepath, v=0):
  ...
  with codecs.open(filepath, 'r', encoding='UTF-8') as f:
    ...
    for i, line in enumerate(f):
      ...
      drc_line = line_parts[0].strip()
      word_info = line_parts[1].strip()
      matches = re.findall('(?:([^\[\s]+) \[(\d+)\.\.\.(\d+)])', word_info)
      if prev_rel_tokens != word_info and len(matches) > 1:
        prev_rel_tokens = word_info
        rel_tokens.append(matches)
      if v >= 5: wrap_print("/{}/".format(drc_line))
      clause = tuple(drc_line.split(' '))
      if len(clause) > 1:  # remove lines that are comments and started with whitespace
        clf.append(clause)
  if clf:  # add last clf
    list_of_clf.append(clf)
    list_of_raw.append(pre_line[3:].strip())
  return (list_of_clf, list_of_raw, rel_tokens)
...
def convert_clf(src, verbose_value, quiet_value, sig_file_value):
  (clfs, raws, list_of_rel_tokens) = file_to_clfs(src, v=verbose_value)
  ...
  return clfs, boxes, op_type_counter, list_of_rel_tokens
...
def convert_sbn(src):
  ...
  with codecs.open(src, 'r', encoding='UTF-8') as f:
    clean_lines = [re.sub(' +', ' ', raw_line[6:]).replace('\n', '') for raw_line in f.readlines() if raw_line.startswith('%SBN%')]
    token_index = -1
    for idx, line in enumerate(clean_lines):
      ...
      if not line_data.sbn_word.isupper():
        token_index += 1
        sbn_data['token_data'].append(line_data)
      sbn_data['raw_data'].append(line_data)

      line_data.index = token_index
      ...
      for part in word_info_parts:
        if part.startswith('--'):
          ...
          word_info.start_idx = indices[0]
          word_info.end_idx = indices[1]
        else:
          word_with_context.append(part)
          word_info.word = part
...
if __name__ == '__main__':
  ...
  drs_folder = 'boxer_data_v2'
  data_folder = to_abspath(base_folder, f'data/box_forms/frank-etal-2013/{drs_folder}')
  for filename in sorted(os.listdir(data_folder)):
    # read clausal forms and raw from the file
    verbose_value = 3
    quiet_value = False
    sig_file_value = ''
    src = to_abspath(data_folder, filename)
    with open(src) as clff:
      file_content = clff.readlines()
      sent = file_content[0][2:].strip()
    ...
    try:
      if file_content:
        clfs, boxes, op_type_counter, rel_tokens = convert_clf(src, verbose_value, quiet_value, sig_file_value)
        clf = clfs[0]
        box_data = boxes[0]
      ...
      wrap_print('X' * 10)
      (clf, box_data, op_type_counter) = ([], {}, Counter())
    wrap_print('- END clf -')
    wrap_print('-' * 100)

    transform_data_to_input_variables(stimuli_ref, filename[:-4], sent, clf, box_data, op_type_counter, sbn_data, rel_tokens)

  today = datetime.today().strftime('%Y%m%d-%H%M%S')
  flush_clean_print_acc(to_abspath(report_folder, drs_folder + f'-box-result_{today}.txt'))
  flush_clean_drs_data(to_abspath(report_folder, drs_folder + f'_data-accumulation_{today}.tsv'))
